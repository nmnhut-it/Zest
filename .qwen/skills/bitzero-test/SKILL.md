...
Use **PSI-based MCP tools** which can read JARs through IntelliJ's indexing.

## MCP Tools for JAR Access

```
mcp_zest_lookupClass        → Get class/method signatures from JARs
mcp_zest_getTypeHierarchy   → See superclasses and interfaces
mcp_zest_findImplementations → Find implementations of interfaces
mcp_zest_findUsages         → See how methods are called
mcp_zest_getCallHierarchy   → Trace callers/callees
mcp_zest_getMethodBody      → Get method implementation details
```

Example workflow:
```
1. lookupClass("ISession")           → See interface methods
2. getTypeHierarchy("ISession")      → Find TestSession, real implementations
3. lookupClass("BaseClientRequestHandler") → See handler contract
4. findImplementations("doLogin")    → See how projects implement login
```
...
```
mcp_zest_validateCode  → Check test compiles
```
