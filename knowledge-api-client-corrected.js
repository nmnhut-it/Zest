...
const KnowledgeAPI = {
    ...
    createKnowledgeBase: async function(name, description, data = null, access_control = null) {
        ...
        try {
            ...
            // Add a small delay to allow the server to fully process the creation
    ...
    knowledgeExists: async function(knowledgeId) {
        try {
            ...
            // If we got a successful response with a collection, it exists
            return result.success && result.collection != null;
        } catch (error) {
            console.error('Error checking if knowledge exists:', error);
            return false;
        }
    },
