...
open class JBCefLocalRequestHandler(
  ...
  private val RESOURCE_REQUEST_HANDLER = object : CefResourceRequestHandlerAdapter() {
    override fun getResourceHandler(browser: CefBrowser?, frame: CefFrame?, request: CefRequest): CefResourceHandler {
      ...
      // Parse URL manually for custom protocols
      val protocolEnd = urlString.indexOf("://")
      if (protocolEnd == -1) return REJECTING_RESOURCE_HANDLER
      
      val protocol = urlString.substring(0, protocolEnd)
      val afterProtocol = urlString.substring(protocolEnd + 3)
      
      val authorityEnd = afterProtocol.indexOf("/")
      val authority = if (authorityEnd == -1) afterProtocol else afterProtocol.substring(0, authorityEnd)
      val path = if (authorityEnd == -1) "" else afterProtocol.substring(authorityEnd)
      
      if (!protocol.equals(myProtocol) || !authority.equals(myAuthority)) {
        return REJECTING_RESOURCE_HANDLER
      }
      
      return try {
        val normalisedPath = path.trim('/')
        myResources[normalisedPath]?.let { it() } ?: REJECTING_RESOURCE_HANDLER
