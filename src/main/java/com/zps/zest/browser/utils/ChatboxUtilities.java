...
public class ChatboxUtilities {
    ...
    /**
     * Sends text to the chat box input field.
     * 
    ...
    /**
     * Clicks the send button to submit the message.
     * 
     * @param project The current project
     * @return true if the operation was successful, false otherwise
     */
    ...
    public static boolean sendTextAndSubmit(Project project, String text) {
        ...
        String script =
                "function sendTextAndSubmit(text) {\n" +
                        ...
                        "    chatInput.innerHTML = '';\n" +
                        "    \n" +
                        "    // Step 4: Add the new text content\n" +
                        "    const p = document.createElement('p');\n" +
                        "    p.textContent = text;\n" +
                        "    chatInput.appendChild(p);\n" +
                        "    // Trigger input event to ensure the UI recognizes the change" + "\n"+
                        ...
                        "    \n" +
                        "    \n" +
                        "    // Step 6: Wait a small amount of time to ensure the UI has updated\n" +
                        "    setTimeout(() => {\n" +
                        "    const sendButton = document.getElementById('send-message-button');\n" +
                        "    if (!sendButton) {\n" +
                        "      console.error('Send button element not found');\n" +
                        "      return false;\n" +
                        "    }\n" +
                        "      // Step 7: Check if button is disabled\n" +
                        "      if (sendButton.disabled) {\n" +
