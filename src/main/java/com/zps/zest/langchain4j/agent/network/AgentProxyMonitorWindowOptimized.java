...
public class AgentProxyMonitorWindowOptimized extends JDialog {
    ...
    private AgentProxyServer.RequestListener requestListener; // Store reference for cleanup
    ...
    public AgentProxyMonitorWindowOptimized(Project project, AgentProxyServer proxyServer, int proxyPort) {
        ...
        setTitle("Agent Proxy Monitor - Port " + proxyPort + " (Close window won't stop server)");
        setModal(false);
        setDefaultCloseOperation(JDialog.DO_NOTHING_ON_CLOSE);

        // Add window listener to confirm closing
            ...
            public void windowClosing(java.awt.event.WindowEvent e) {
                int result = Messages.showYesNoDialog(
                        ...
                        "Close the monitor window?\n\n" +
                                "The proxy server will continue running on port " + proxyPort + ".\n" +
                                "You can reopen this monitor from the Zest menu.",
                        "Close Monitor",
    ...
    private JPanel createBottomPanel() {
        ...
        JButton closeButton = createButton("Close Monitor (Server Continues)", this::dispose);
        closeButton.setToolTipText("Close this monitor window. The proxy server will continue running.");
    ...
    private void startMonitoring() {
        ...
        // Create and register request listener
        requestListener = new AgentProxyServer.RequestListener() {
            @Override
            public void onRequestStarted(String requestId, String endpoint, String method) {
                if (isDisposed || executor.isShutdown()) {
                    return; // Don't process if window is disposed or executor is shut down
                }

                executor.submit(() -> {
                    if (isDisposed) return; // Double-check inside the task
                    ...
                    SwingUtilities.invokeLater(() -> {
                        if (isDisposed) return; // Check again in UI thread
            ...
            public void onToolExecuted(String requestId, String toolName, boolean success, String result) {
                if (isDisposed) return; // Check before processing
        ...
        proxyServer.addRequestListener(requestListener);
    }
    ...
    private void checkServerStatus() {
        if (isDisposed || executor.isShutdown()) return;
        ...
        executor.submit(() -> {
            ...
            try {
                ...
                SwingUtilities.invokeLater(() -> {
                    if (isDisposed) return; // Check in UI thread
    ...
    private void stopServer() {
        int result = Messages.showYesNoDialog(
                ...
                "Are you sure you want to stop the Agent Proxy Server?\n\n" +
                        "Note: You can close this monitor window without stopping the server.",
                "Stop Server",
                Messages.getQuestionIcon()
        );

        if (result == Messages.YES) {
            // Call the static stopServer method in StartAgentProxyAction to properly stop everything
            StartAgentProxyAction.stopServer();
        }
    }
    ...
    public void dispose() {
        isDisposed = true;
        log("Monitor window closing. Proxy server continues running on port " + proxyPort);
        cleanup();
        super.dispose();
    }

    private void cleanup() {
        // Remove the request listener first to prevent new tasks
        if (requestListener != null && proxyServer != null) {
            proxyServer.removeRequestListener(requestListener);
        }
