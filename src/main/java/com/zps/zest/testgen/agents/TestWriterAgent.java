...
public class TestWriterAgent extends StreamingBaseAgent {
    ...
    public com.zps.zest.testgen.snapshot.AgentSnapshot exportSnapshot(String sessionId, String description, String originalPrompt) {
        LOG.info("Exporting TestWriterAgent snapshot for session: " + sessionId);

        MessageWindowChatMemory memory = getChatMemory();
        ...
        return new com.zps.zest.testgen.snapshot.AgentSnapshot(
            ...
            com.zps.zest.testgen.snapshot.AgentType.TEST_WRITER,
            sessionId,
    ...
    public void restoreFromSnapshot(com.zps.zest.testgen.snapshot.AgentSnapshot snapshot) {
        LOG.info("Restoring TestWriterAgent from snapshot: " + snapshot.getSessionId());

        if (snapshot.getAgentType() != com.zps.zest.testgen.snapshot.AgentType.TEST_WRITER) {
            throw new IllegalArgumentException("Cannot restore TestWriterAgent from snapshot of type: " + snapshot.getAgentType());
        }
        ...
        LOG.info("Restored TestWriterAgent state: " + memory.messages().size() + " messages");
    }
