...
/**
 * Custom dialog for showing method rewrite diff with prominent Accept/Reject buttons
 */
class MethodRewriteDiffDialog(
...
) : DialogWrapper(project, false) {  // Non-modal - allows working in editor while diff is open
    
    companion object {
        private val logger = Logger.getInstance(MethodRewriteDiffDialog::class.java)
        private const val CONTEXT_LINES = 100
        ...
        fun show(
            ...
            ApplicationManager.getApplication().invokeLater {
                val dialog = MethodRewriteDiffDialog(
    ...
    init {
        ...
        setOKButtonIcon(AllIcons.Actions.Commit)
        setCancelButtonText("Reject Changes") 
        init()
    ...
    private fun getContentWithContext(): Pair<String, String> {
        ...
        val contextStartLine = maxOf(0, startLine - CONTEXT_LINES)
        val contextEndLine = minOf(document.lineCount - 1, endLine + CONTEXT_LINES)
        ...
        val extendedOriginal = buildString {
        ...
        val extendedModified = buildString {
            if (contextStartLine > 0) {
                appendLine("// ... ${contextStartLine} lines before ...")
                appendLine()
            }
        ...
        return Pair(extendedOriginal, extendedModified)
    }
    ...
    private fun findButton(container: Container, text: String): JButton? {
        for (component in container.components) {
            if (component is JButton && component.text == text) {
                ...
                findButton(component, text)?.let { return it }
            }
        }
        return null
    }
